...
function loadProgress() {
  ...
  try {
    ...
    completedRows = (progress.completedRows ?? []).filter((rowIndex) => rowIndex < pattern.rows.length);
...
function renderNotes() {
  ...
  turnInstructionNote.textContent = stitchMode === "HDC"
...
function renderCurrentRow() {
  ...
  if (!row || !title || !instructions || !total || !direction || !chartContainer || !badge || !stitchSummary) {
    return;
  }
...
function updateGraphHighlight() {
  ...
  if (!graphImage || !graphHighlight || !graphImage.complete) {
    return;
  }
